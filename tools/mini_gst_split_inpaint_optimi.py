...
def build_pipeline_desc(args):
    ...
    # cola de conversión y (opcional) resize
    # IMPORTANTE: no dejes espacios “raros” antes de video/x-raw
    if getattr(args, "gst_w", None) and getattr(args, "gst_h", None):
        tail = (
    ...
    if getattr(args, "rtp", False):
        caps = args.rtp_caps or 'application/x-rtp, media=video, encoding-name=H264, payload=96'
        if dec == "decodebin":
            pipe = (
                f'udpsrc port={args.port} caps="{caps}" ! '
                "rtpjitterbuffer do-lost=true ! rtph264depay ! h264parse ! "
                f"decodebin ! {tail} ! {sink}"
            )
        else:
            pipe = (
                f'udpsrc port={args.port} caps="{caps}" ! '
...
def detect_corruption_mask(frame_bgr, prev_bgr=None, block=16, var_th=2.0, diff_th=18.0,
                           proc_scale=1.0):
    # downscale para procesar más rápido
    if proc_scale != 1.0:
        small = cv2.resize(frame_bgr, None, fx=proc_scale, fy=proc_scale,
                           interpolation=cv2.INTER_AREA)
        prev_s = cv2.resize(prev_bgr, None, fx=proc_scale, fy=proc_scale,
                            interpolation=cv2.INTER_AREA) if prev_bgr is not None else None
        mask_s = _detect_mask_core(small, prev_s, max(1,int(block*proc_scale)),
                                   var_th, diff_th)
        # reescala máscara a tamaño original
        mask = cv2.resize(mask_s, (frame_bgr.shape[1], frame_bgr.shape[0]),
                          interpolation=cv2.INTER_NEAREST)
        return mask
    else:
        return _detect_mask_core(frame_bgr, prev_bgr, block, var_th, diff_th)
...
def main():
    ...
    p.add_argument("--decoder", default="decodebin",
    ...
    try:
        while True:
            ...
            # DETECCIÓN (downscale interno)
            mask = detect_corruption_mask(raw, prev_bgr=prev,
            ...
            if args.mode == "mask":
                ...
                if ratio >= args.small_thresh and np.any(mask):
                    if args.roi_inpaint:
                        # --- ROI inpaint REAL (solo procesa el área afectada) ---
                        ...
                        if len(xs) > 0 and len(ys) > 0:
                            ...
                            if args.proc_scale < 1.0:
                                # inpaint a menor resolución, solo en el ROI
                                ...
                                roi_repaired = cv2.inpaint(roi_frame, roi_mask, args.inpaint_radius,
                                                           args.inpaint_method)

                            # reemplazamos solo la región afectada
                            right = raw.copy()
                            right[y1:y2, x1:x2] = roi_repaired
                        else:
                            # no hay área dañada -> frame intacto
                            right = raw
                    else:
                        # --- full-frame inpaint ---
                        if args.proc_scale < 1.0:
                            # procesamos a menor escala solo una vez
